...
Template.projects.onCreated(function () {
  ...
  templateObject.projectID = new ReactiveVar(0);
...
Template.projects.onRendered(function () {
  ...
  templateObject.projectID.set(currentId);
  $('#addProjectID').val(currentId);
  ...
  templateObject.getAllTaskList = function () {
    crmService.getAllTaskList(currentId).then(function (data) {
      if (data.tprojecttasks && data.tprojecttasks.length > 0) {
        ...
        all_projectrecords = data.tprojecttasks.filter(item => item.fields.ProjectID == currentId);
        ...
        templateObject.allrecords.set(all_projectrecords);
  ...
  templateObject.getTProjectList = function () {
    crmService.getTProjectList().then(function (data) {
      if (data.tprojectlist && data.tprojectlist.length > 0) {
        ...
        let projectID = templateObject.projectID.get();
...
Template.projects.events({
      ...
      if (data.fields.ID == id) {
        ...
        var objDetails = {
          ...
          fields: {
            ...
            ProjectID: templateObject.projectID.get(),
  ...
  'click .btnSaveEdit': function (e) {
    ...
    let projectID = $('#editProjectID').val() ? $('#editProjectID').val() : templateObject.projectID.get();
  ...
  'click .btnSaveAddTask': function (e) {
    ...
    $('.fullScreenSpin').css('display', 'inline-block');
    let projectID = $('#addProjectID').val() ? $('#addProjectID').val() : templateObject.projectID.get();
