      ...
      if(dataObject.length == 0){
        salesService.getAllQuoteList().then(function (data) {
          ...
          for(let i=0; i<data.tquote.length; i++){
            ...
            let totalAmount = utilityService.modifynegativeCurrencyFormat(data.tquote[i].TotalAmountInc)|| 0.00;
            let totalPaid = utilityService.modifynegativeCurrencyFormat(data.tquote[i].TotalPaid)|| 0.00;
            let totalOutstanding = utilityService.modifynegativeCurrencyFormat(data.tquote[i].TotalBalance)|| 0.00;
            var dataList = {
              id: data.tquote[i].ID || '',
              employee:data.tquote[i].EmployeeName || '',
              sortdate: data.tquote[i].SaleDate !=''? moment(data.tquote[i].SaleDate).format("YYYY/MM/DD"): data.tquote[i].SaleDate,
              saledate: data.tquote[i].SaleDate !=''? moment(data.tquote[i].SaleDate).format("DD/MM/YYYY"): data.tquote[i].SaleDate,
              duedate: data.tquote[i].DueDate !=''? moment(data.tquote[i].DueDate).format("DD/MM/YYYY"): data.tquote[i].DueDate,
              customername: data.tquote[i].CustomerName || '',
              totalamountex: totalAmountEx || 0.00,
              totaltax: totalTax || 0.00,
              totalamount: totalAmount || 0.00,
              totalpaid: totalPaid || 0.00,
              totaloustanding: totalOutstanding || 0.00,
              salestatus: data.tquote[i].SalesStatus || '',
              custfield1: data.tquote[i].SaleCustField1 || '',
              custfield2: data.tquote[i].SaleCustField2 || '',
              comments: data.tquote[i].Comments || '',
          };
        if(data.tquote[i].CustomerName != ''){
          ...
          setTimeout(function () {
              $('#tblquotelist').DataTable({
                    ...
                    buttons: [
                       ...
                       filename: "quotelist_"+ moment().format(),
