    ...
    templateObject.getGeneralLedgerReports = function (dateFrom, dateTo, ignoreDate) {
      if(!localStorage.getItem('VS1GeneralLedger_Report')){
        ...
        if(data.tgeneralledgerreport.length){
          ...
          recordObj.dataArr = [
              ...
              utilityService.modifynegativeCurrencyFormat(data.tgeneralledgerreport[i].AMOUNTINC) || '-',
              // utilityService.modifynegativeCurrencyFormat(data.tgeneralledgerreport[i].Current) || '-',
              utilityService.modifynegativeCurrencyFormat(data.tgeneralledgerreport[i].DEBITSEX) || '-',
              utilityService.modifynegativeCurrencyFormat(data.tgeneralledgerreport[i].CREDITSEX) || '-',
      ...
      for (let i = 0; i < allRecords.length; i++) {
          ...
          for (let k = 0; k < allRecords[i][1].data.length; k++) {
              ...
              Currenttotal = Currenttotal + utilityService.convertSubstringParseFloat(allRecords[i][1].data[k].dataArr[6]);
              oneMonth = oneMonth + utilityService.convertSubstringParseFloat(allRecords[i][1].data[k].dataArr[7]);
              twoMonth = twoMonth + utilityService.convertSubstringParseFloat(allRecords[i][1].data[k].dataArr[8]);
...
let grandOlder = 0;
    ...
    for (let n = 0; n < current.length; n++) {
        ...
        //for (let m = 0; m < current[n].data.length; m++) {
             ...
             grandthreeMonth = grandthreeMonth + utilityService.convertSubstringParseFloat(current[n][7]);
             grandOlder = grandOlder + utilityService.convertSubstringParseFloat(current[n][8]);
