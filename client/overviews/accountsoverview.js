        ...
        accountService.getAccountListVS1().then(function (data) {
          ...
          for(let i=0; i<data.taccountvs1.length; i++){
             ...
             if (!isNaN(data.taccountvs1[i].Balance)) {
               ...
               extra: data.taccountvs1[i].Extra || '',
  ...
  'click .resetTable' : function(event){
    ...
    if(getcurrentCloudDetails){
      if (getcurrentCloudDetails._id.length > 0) {
        ...
        var checkPrefDetails = CloudPreference.findOne({userid:clientID,PrefName:'tblAccountOverview'});
        if (checkPrefDetails) {
      ...
      if (getcurrentCloudDetails._id.length > 0) {
        ...
        if (checkPrefDetails) {
          CloudPreference.update({_id: checkPrefDetails._id},{$set: { userid: clientID,username:clientUsername,useremail:clientEmail,
            PrefGroup:'salesform',PrefName:'tblAccountOverview',published:true,
...
'click .btnDeleteAccount': function () {
  ...
  Template.accountsoverview.helpers({
    ...
    return CloudPreference.findOne({userid:Session.get('mycloudLogonID'),PrefName:'tblAccountOverview'});
  },
  accounttyperecords: () => {
