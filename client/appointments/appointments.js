        ...
        var calendar = new Calendar(calendarEl, {
            ...
            eventDrop: function (info) {
                if (info.event._def.publicId != "") {
                    ...
                    let startTime = ("0" + dateStart.getHours()).toString().slice(-2) + ':' + ("0" + dateStart.getMinutes()).toString().slice(-2);
                    let endTime = ("0" + dateEnd.getHours()).toString().slice(-2) + ':' + ("0" + dateStart.getMinutes()).toString().slice(-2);
                    let index = appointmentData.map(function (e) { return e.id; }).indexOf(parseInt(eventDropID));
                    let resourceIndex = resourceData.map(function (e) { return e.employeeName;}).indexOf(appointmentData[index].employeename);
                    var result = appointmentData.filter(apmt => { return apmt.id == eventDropID });
                    if (result.length > 0) {
                        objectData = {
                            type: "TAppointment",
                            fields: {
...
$(document).on("mouseenter", "#tblEmployeeSideList tbody tr", function () {
    ...
    document.addEventListener("drop", function (event) {
        ...
        let index = appointmentData.map(function (e) { return e.id; }).indexOf(parseInt(id));
        let calendarSet = templateObject.globalSettings.get();
        let hideDays = '';
        let slotMin = "06:00:00";
        let slotMax = "21:00:00";
