...
Template.journalentrycard.onRendered(() => {
    ...
    $(document).ready(function () {
        ...
        //         text: "Do you want to Save or Cancel this Journal Entry?",
        //         type: 'info',
    ...
    $(document).on("click", "#tblAccount tbody tr", function (e) {
        ...
        if (selectLineID) {
            ...
            $('#' + selectLineID + " .lineAccountNo").text(lineAccoutNo);
...
Template.journalentrycard.helpers({
    ...
    purchaseCloudPreferenceRec: () => {
        return CloudPreference.findOne({ userid: Session.get('mycloudLogonID'), PrefName: 'journalentrycard' });
    },
...
Template.journalentrycard.events({
    ...
    'blur .lineDebitEx': function (event) {
        //if (event.which >= 48 && event.which <= 57) {
                ...
                if (checkPrefDetails) {
                    CloudPreference.update({ _id: checkPrefDetails._id }, {
                        $set: {
                            ...
                            PrefGroup: 'purchaseform', PrefName: 'tblJournalEntryLine', published: true,
    ...
    'click .btnResetSettings': function (event) {
        ...
        if (getcurrentCloudDetails) {
            if (getcurrentCloudDetails._id.length > 0) {
                ...
                var clientUsername = getcurrentCloudDetails.cloudUsername;
                var clientEmail = getcurrentCloudDetails.cloudEmail;
                var checkPrefDetails = CloudPreference.findOne({ userid: clientID, PrefName: 'journalentrycard' });
                if (checkPrefDetails) {
                    CloudPreference.remove({ _id: checkPrefDetails._id }, function (err, idTag) {
                        if (err) {
