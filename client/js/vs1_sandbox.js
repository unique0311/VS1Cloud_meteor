...
Template.signin.onRendered(function(){
  ...
  if ((Session.get('sidePanelToggle') === '') || (!Session.get('sidePanelToggle'))) {
...
*/
  ...
  function getAccessLevelData(userAccessOptions){
    ...
    $.each(userAccessOptions, function (itemaccess, optionaccess) {
          // console.log(optionaccess.FormId);
          lineItemObjlevel = {
          formID: optionaccess.FormId || '',
          accessLevel: optionaccess.AccessLevel || '',
          accessLevelname: optionaccess.AccessLevelName || '',
          description: optionaccess.Description || '',
          formName: optionaccess.FormName || '',
          accessID: optionaccess.Id || '',
      };

      if(optionaccess.AccessLevel === 1){
        if(optionaccess.Description === "Print Delivery Docket"){
          isDocket = true;
        }
        if(optionaccess.Description === "Print Invoice"){
          isInvoice = true;
        }

        if(optionaccess.Description === "User Password Details"){
          isUserPassDetail = true;
        }

        if(optionaccess.Description === "Dashboard"){
          isDashboard = true;
        }
        if(optionaccess.Description === "Main"){
          isMain = true;
        }
        if(optionaccess.Description === "Inventory" || optionaccess.Description === "Inventory Tracking"){
          isInventory = true;
        }
        if(optionaccess.Description === "Manufacturing"){
          isManufacturing = true;
        }

        if(optionaccess.Description === "Access Levels"){
          isAccessLevels = true;
        }
        if(optionaccess.Description === "Shipping"){
          isShipping = true;
        }
        if(optionaccess.Description === "Stock Transfer"){
          isStockTransfer = true;
        }
        if(optionaccess.Description === "Side Panel Menu"){
          isSidePanel = true;
          isSidePanelID = optionaccess.ID;
          isSidePanelFormID = optionaccess.FormId;
        }
        if(optionaccess.Description === "Top Panel Menu"){
          isTopPanel = true;
          isTopPanelID = optionaccess.ID;
          isTopPanelFormID = optionaccess.FormId;
        }
        if(optionaccess.Description === "Stock Take"){
          isStockTake = true;
        }
        if(optionaccess.Description === "Sales"){
          isSales = true;
        }
        if(optionaccess.Description === "Purchases"){
          isPurchases = true;
        }
        if(optionaccess.Description === "Expense Claims"){
          isExpenseClaims = true;
        }
        if(optionaccess.Description === "Fixed Assets"){
          isFixedAssets = true;
        }
        if(optionaccess.Description === "Payments"){
          isPayments = true;
        }
        if(optionaccess.Description === "Contacts"){
          isContacts = true;
        }
        if(optionaccess.Description === "Accounts"){

          isAccounts = true;
        }
        if(optionaccess.Description === "Reports"){
          isReports = true;
        }
        if(optionaccess.Description === "Settings"){
          isSettings = true;
        }
        if(optionaccess.Description === "View Dockets"){
          isViewDockets = true;
        }

        if(optionaccess.Description === "Qty Only on Purchase Order"){
          isPurchaseQtyOnly = true;
        }

        if(optionaccess.Description === "Qty Only on Sales"){
          isSalesQtyOnly = true;
        }
...
// alert(userAccessOptions[0].EmployeeId);
    let userSerssion = {'loggedEmpID':userAccessOptions[0].EmployeeId,
