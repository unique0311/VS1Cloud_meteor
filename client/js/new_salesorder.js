                                ...
                                if (useData[d].fields.Lines.length) {
                                    for (let i = 0; i < useData[d].fields.Lines.length; i++) {
                                        ...
                                        lineItemObj = {
                                            ...
                                            unitPrice: currencySymbol + '' + useData[d].fields.Lines[i].fields.LinePrice.toLocaleString(undefined, { minimumFractionDigits: 2 }) || 0,
                                    ...
                                    lineItemObj = {
                                        ...
                                        unitPrice: useData[d].fields.Lines[i].fields.LinePrice.toLocaleString(undefined, { minimumFractionDigits: 2 }) || 0,
...
Template.new_salesorder.helpers({
    ...
    salesCloudPreferenceRec: () => {
        return CloudPreference.findOne({ userid: Session.get('mycloudLogonID'), PrefName: 'new_salesorder' });
    },
    salesCloudGridPreferenceRec: () => {
        return CloudPreference.findOne({ userid: Session.get('mycloudLogonID'), PrefName: 'tblSalesOrderLine' });
    },
    uploadedFiles: () => {
    ...
    'click .btnSaveGridSettings': function(event) {
        ...
        if (getcurrentCloudDetails) {
            if (getcurrentCloudDetails._id.length > 0) {
                ...
                var clientUsername = getcurrentCloudDetails.cloudUsername;
                var clientEmail = getcurrentCloudDetails.cloudEmail;
                var checkPrefDetails = CloudPreference.findOne({ userid: clientID, PrefName: 'tblSalesOrderLine' });
                if (checkPrefDetails) {
                    CloudPreference.update({ _id: checkPrefDetails._id }, {
                        $set: {
                            userid: clientID,
