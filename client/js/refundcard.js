...
Template.refundcard.onRendered(()=>{
    ...
    //let salesCloudGridPreferenceRec = CloudPreference.findOne({userid:Session.get('mycloudLogonID'),PrefName:'tblInvoiceLine'});
    ...
    if(url.indexOf('?id=') > 0){
        ...
        //console.log(templateObject.invoicerecord.get());
        // alert('No ID');
    }

    // $('.tblInvoiceLine').dragableColumns();
    ...
    $(document).on("click", "#tblInventory tbody tr", function(e) {
        ...
        if(selectLineID){
            ...
            lineAmount = '-'+lineUnitPrice.replace(/[^0-9.-]+/g,"") || 0;
            ...
            $tblrows.each(function (index) {
                ...
                if (!isNaN(subTotal)) {
                    ...
                    document.getElementById("subtotal_total").innerHTML = utilityService.modifynegativeCurrencyFormat(subGrandTotal.toLocaleString(undefined, {minimumFractionDigits: 2}));
                }

                if (!isNaN(taxTotal)) {
                    taxGrandTotal += isNaN(taxTotal) ? 0 : taxTotal;
                    document.getElementById("subtotal_tax").innerHTML = utilityService.modifynegativeCurrencyFormat(taxGrandTotal.toLocaleString(undefined, {minimumFractionDigits: 2}));
                }

                if (!isNaN(subGrandTotal) && (!isNaN(taxGrandTotal))) {
                    let GrandTotal = (parseFloat(subGrandTotal)) + (parseFloat(taxGrandTotal));
                    document.getElementById("grandTotal").innerHTML = utilityService.modifynegativeCurrencyFormat(GrandTotal.toLocaleString(undefined, {minimumFractionDigits: 2}));
                    document.getElementById("balanceDue").innerHTML = utilityService.modifynegativeCurrencyFormat(GrandTotal.toLocaleString(undefined, {minimumFractionDigits: 2}));
                    document.getElementById("totalBalanceDue").innerHTML = utilityService.modifynegativeCurrencyFormat(GrandTotal.toLocaleString(undefined, {minimumFractionDigits: 2}));
...
Template.refundcard.helpers({
    ...
    salesCloudPreferenceRec: () => {
        return CloudPreference.findOne({userid:Session.get('mycloudLogonID'),PrefName:'refundcard'});
    },
...
Template.refundcard.events({
    ...
    'change .lineUnitPrice': function(event){
        ...
        if (!isNaN($(event.target).val())){
            ...
            $(event.target).val(utilityService.modifynegativeCurrencyFormat(inputUnitPrice.toLocaleString(undefined, {minimumFractionDigits: 2})));
        }else{
            ...
            salesService.saveInvoiceEx(objDetails).then(function (objDetails) {
                ...
                if(getcurrentCloudDetails){
                    if (getcurrentCloudDetails._id.length > 0) {
                        ...
                        var checkPrefDetails = CloudPreference.findOne({userid:clientID,PrefName:'refundcard'});
                        ...
                        if (checkPrefDetails) {
                            CloudPreference.update({ _id: checkPrefDetails._id},{ $set: {username:clientUsername,useremail:clientEmail,
                                                                                         PrefGroup:'salesform',PrefName:'refundcard',published:true,
                                                                                         customFields:[{
                                                                                             index: '1',
            ...
            if (getcurrentCloudDetails._id.length > 0) {
                ...
                if (checkPrefDetails) {
                    CloudPreference.update({_id: checkPrefDetails._id},{$set: { userid: clientID,username:clientUsername,useremail:clientEmail,
                                                                               PrefGroup:'salesform',PrefName:'tblInvoiceLine',published:true,
