...
Template.creditcard.onRendered(() => {
    ...
    if (url.indexOf('?id=') > 0) {
                ...
                getVS1Data('TCredit').then(function(dataObject) {
                    if (dataObject.length == 0) {
                      purchaseService.getOneCreditData(currentCredit).then(function(data) {
                          ...
                          // alert(JSON.stringify(templateObject.creditrecord.get()));
                          ...
                          if (templateObject.creditrecord.get()) {
                              ...
                              // $('#tblCreditLine').colResizable({
                              //   liveDrag:true});
                              //$('#tblCreditLine').removeClass('JColResizer');
        ...
        templateObject.creditrecord.set(creditrecord);
        if (templateObject.creditrecord.get()) {
            // $('#tblCreditLine').colResizable({liveDrag:true});
            Meteor.call('readPrefMethod', Session.get('mycloudLogonID'), 'tblCreditLine', function(error, result) {
                if (error) {
                    console.log(error);
                    //Bert.alert('<strong>Error:</strong> user-not-found, no user found please try again!', 'danger');
        ...
        //console.log(templateObject.creditrecord.get());
        // alert('No ID');
    }

    // $('.tblCreditLine').dragableColumns();
