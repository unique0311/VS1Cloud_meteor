...
Template.depositcard.onCreated(()=>{
    ...
    // templateObject.arearecords = new ReactiveVar();
...
Template.depositcard.onRendered(()=>{
    ...
    //   purchaseService.getAreaCode().then(function(data){
    //       for(let i in data.tareacode){
    //
    //           let arearecordsObj = {
    //               areacode: data.tareacode[i].Areacode || ' ',
    //           };
    //
    //           arearecords.push(arearecordsObj);
    //           templateObject.arearecords.set(arearecords);
    //
    //       }
    //   });
    // }
...
Template.depositcard.helpers({
    ...
    // arearecords: () => {
    //     return Template.instance().arearecords.get().sort(function(a, b){
    //         if (a.areacode == 'NA') {
    //             return 1;
    //         }
    //         else if (b.areacode == 'NA') {
    //             return -1;
    //         }
    //         return (a.areacode.toUpperCase() > b.areacode.toUpperCase()) ? 1 : -1;
    //     });
    // },
    termrecords: () => {
...
Template.depositcard.events({
    ...
    'click .btnSave': function(event){
        ...
        if (account === ''){
          ...
          if($('#depositTotal').val().replace(/[^0-9.-]+/g,"")!= $('#depositTotalLine').text().replace(/[^0-9.-]+/g,"")){
              // swal('Total Deposit does not equal Total Deposited!', '', 'warning');
              swal({
                  ...
                  if (result.value) {
                    ...
                    if(getso_id[1]){
                        $('#tblDepositEntryLine > tbody > tr').each(function(){
                            var lineID = this.id;
                            let tdaccount = $('#'+lineID+" .lineAccountName").text() || $('#sltAccountName').val();
                            let tdpaymentmethod = $('#'+lineID+" .colPaymentMethod").text();
    ...
    'click .colAccount': function(event) {
        $('#tblDepositEntryLine tbody tr .colAccount').attr("data-toggle", "modal");
        $('#tblDepositEntryLine tbody tr .colAccount').attr("data-target", "#productListModal");
        var targetID = $(event.target).closest('tr').attr('id'); // table row ID
        $('#selectLineID').val(targetID);
        // Autofocus Searchbar
        setTimeout(function() {
