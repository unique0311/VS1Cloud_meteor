    ...
    getAllProducts = async function () {
       let data= await productService.getProductList();
          ...
          var recentTranObject = {
                      ...
                      purchasesPrice:  utilityService.modifynegativeCurrencyFormat(data.tproduct[i].BuyQty1CostInc) || 0,
                      ...
                      salesPrice: utilityService.modifynegativeCurrencyFormat(data.tproduct[i].SellQty1PriceInc) || 0,
      ...
      // alert('ere');
        productService.getRecentTransactions().then(function (productData) {
           ...
           for (let j = 0; j < listId.length; j++) {
                for (let i =0; i < productData.tproductsalesdetailsreport.length; i++) {
                    if (productData.tproductsalesdetailsreport[i].ProductID === listId[j].id) {
                        var recentTranObject = {
                            productId: productData.tproductsalesdetailsreport[i].ProductID,
                            productName: productData.tproductsalesdetailsreport[i].ProductName,
                            productDescription: productData.tproductsalesdetailsreport[i].ProductDescription,
                            purchasesPrice:  listId[j].purchasePrc,
                            purchaseAccount: productData.tproductsalesdetailsreport[i].TransactionType,
                            costOfGoodsSoldAccount: productData.tproductsalesdetailsreport[i]['Total Profit (Inc)'] || 0,
                            unitOfMeasure: productData.tproductsalesdetailsreport[i].UnitOfMeasure || ' ',
