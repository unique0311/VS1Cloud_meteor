...
Template.paymentcard.events({
   ...
   'click .btnSelectSuppliers': function(event) {
     ...
     if(selectedSupplierPayments.length == 1) {
        let selectedId =  $('#rowID').val();
        $('#rowID').val();
        $('#' + selectedId + " .colTransDate").text(selectedSupplierPayments[0].date);
        $('#' + selectedId + " .colType").text(selectedSupplierPayments[0].type);
        $('#' + selectedId + " .colTransNo").text(selectedSupplierPayments[0].orderNo);
        $('#' + selectedId + " .lineOrginalamount").text(selectedSupplierPayments[0].originalAmount);
        $('#' + selectedId + " .lineAmountdue").text(selectedSupplierPayments[0].outstandingAmount);
        $('#' + selectedId + " .linePaymentamount ").val(selectedSupplierPayments[0].paymentAmount);
        $('#' + selectedId + " .lineOutstandingAmount").text(selectedSupplierPayments[0].outstandingAmount);
        $('#' + selectedId + " .colComments").text(selectedSupplierPayments[0].comments);
        $('#rowID').val(selectedSupplierPayments[0].awaitingId);
        let currentApplied = $('.lead').text().replace('$','');
        var currentApplied = parseFloat(currentApplied.match(/-?(?:\d+(?:\.\d*)?|\.\d+)/)[0]);
        let total = currentApplied + parseFloat($('#' + selectedId + " .linePaymentamount ").val().replace('$','').replace(',',''));
        total = Currency+total
        $('.appliedAmount').text(total);
        $('#' + selectedId + " .colTransDate").closest('tr').removeAttr('id').removeAttr('name').attr('id',selectedSupplierPayments[0].awaitingId).attr('name',selectedSupplierPayments[0].awaitingId);
        $('#supplierPaymentListModal').modal('hide');
     }else if(selectedSupplierPayments.length > 1) {
      $('#tblSupplierPaymentcard tbody>tr:last').remove();
      let currentApplied = $('.lead').text().replace('$','');
      var currentApplied = parseFloat(currentApplied.match(/-?(?:\d+(?:\.\d*)?|\.\d+)/)[0])
      let total = parseFloat(currentApplied);
       for(let x=0; x < selectedSupplierPayments.length; x++) {
        ...
        $('.appliedAmount').text(Currency+total);
        $('#supplierPaymentListModal').modal('hide');
